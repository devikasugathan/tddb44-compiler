...
prog_head       : T_PROGRAM T_IDENT
                {
                    ...
                    $$ = new ast_procedurehead(pos, sym_p); // procedurehead node(info abt env) with pos info and sym_index
                    sym_tab->open_scope();
                }
                ;
...
const_decl      : T_IDENT T_EQ integer T_SEMICOLON
                {
                    ...
                    position_information *pos =
                        new position_information(@1.first_line,
                                                 @1.first_column);
                    sym_tab->enter_constant(pos,$1,integer_type,$3->value); //(pos,poolindex ident, type, value of const )
                ...
                | T_IDENT T_EQ const_id T_SEMICOLON
                {
                    ...
                    position_information *pos =
                        new position_information(@1.first_line,
                                                 @1.first_column);
                    symbol *temp = sym_tab->get_symbol($3->sym_p);
                    // Make sure it exists. The compiler would crash if null returned.
                    if(temp==NULL || temp->tag != SYM_CONST) {
                        type_error(pos) << "bad index in constant declaration:"
                                        << yytext << endl << flush;
                    }else {
                        constant_symbol *cons = temp->get_constant_symbol();
                        if(cons->type == integer_type){
                            sym_tab->enter_constant(pos,$1,cons->type,cons->const_value.ival);
                        } else if(cons->type == real_type){
                            sym_tab->enter_constant(pos,$1,cons->type,cons->const_value.rval);
...
var_decl        : T_IDENT T_COLON type_id T_SEMICOLON
                {
                    ...
                    position_information *pos =
                        new position_information(@1.first_line,
                                                 @1.first_column);
                    sym_tab->enter_variable(pos, $1, $3->sym_p);//pos,ident,sym_index to the desired type
                }
                | T_IDENT T_COLON T_ARRAY T_LEFTBRACKET integer T_RIGHTBRACKET T_OF type_id T_SEMICOLON
                {
                    ...
                    position_information *pos =
                        new position_information(@1.first_line,
                                                 @1.first_column);
                    sym_tab->enter_array(pos, $1, $8->sym_p, $5->value); //pos,ident, sym_index to desired type array cardinality
                }
...
func_decl       : func_head opt_param_list T_COLON type_id T_SEMICOLON const_part variable_part
                {
                    ...
                    sym_tab->set_symbol_type($1->sym_p,$4->sym_p); //setting symbol to the desired type ie get type of symbol and set it to the type given
                    $$ =$1;
                }
                ;
...
opt_param_list  : T_LEFTPAR param_list T_RIGHTPAR
                ...
                | T_LEFTPAR error T_RIGHTPAR
                {
                    position_information *pos =
                        new position_information(@1.first_line,
                                                 @1.first_column);
                    type_error(pos) << "Error Param_list"<<yytext<<endl<<flush;
                }
                | /* empty */
                {
                    /* Your code here */
...
stmt_list       : stmt
                {
                    ...
                    position_information *pos =
                        new position_information(@1.first_line,
                                                 @1.first_column);
                    if($1 != NULL) {
                        $$ = new ast_stmt_list(pos,$1); // first elem of list
                    } else {
                        $$ = NULL;
                    }
                }
                | stmt_list T_SEMICOLON stmt
                {
                    ...
                    position_information *pos =
                        new position_information(@1.first_line,
                                                 @1.first_column);
                    if($3 != NULL){
                        $$ = new ast_stmt_list(pos,$3,$1);  //constructor to add new stmt to stmt list
                    } else {
                        $$ = $1;
                    }
                }
                | stmt_list error stmt
                {
                    position_information *pos =
                        new position_information(@1.first_line,
                                                 @1.first_column);
                    type_error($1->pos) <<"Semicolon"<<yytext<<endl<<flush;
...
stmt            : T_IF expr T_THEN stmt_list elsif_list else_part T_END
                {
                    ...
                    position_information *pos =
                        new position_information(@1.first_line,
                                                 @1.first_column);
                    $$ = new ast_if(pos, $2, $4, $5, $6); // pos, expr, stmt list*, elsif list*
                }
                | T_IF expr error stmt_list elsif_list else_part T_END
                {
                    ...
                    position_information *pos =
                        new position_information(@1.first_line,
                                                 @1.first_column);
                    type_error(pos) <<"Semicolon"<<yytext<<endl<<flush;
                }
                | T_WHILE expr T_DO stmt_list T_END
                {
                    ...
                    position_information *pos =
                        new position_information(@1.first_line,
                                                 @1.first_column);
                    $$ = new ast_while(pos, $2, $4);  //pos, expr, stmt list
                ...
                | proc_id T_LEFTPAR opt_expr_list T_RIGHTPAR
                {
                    ...
                    position_information *pos =
                        new position_information(@1.first_line,
                                                 @1.first_column);
                    $$ = new ast_procedurecall(pos, $1, $3); //eg fie(1)
                }
                | proc_id T_LEFTPAR error T_RIGHTPAR
                {
                    $$ = NULL;
                    yyerror("opt_expr_list error proc id stmt");
                }
                ...
                | lvariable T_ASSIGN expr
                {
                    ...
                    position_information *pos =
                        new position_information(@1.first_line,
                                                 @1.first_column);
                    $$ = new ast_assign(pos, $1, $3); //pos, lval,expr
                }
...
lvariable       : lvar_id
                ...
                {
                    $$ = NULL;
                    yyerror("Lvariable error Array index");
                }
                ;
...
rvariable       : rvar_id
                ...
                {
                    /* Your code here */
                    $$ = new ast_indexed($1->pos, $1, $3); //pos, arrayid, expr
                }
                | array_id T_LEFTBRACKET error T_RIGHTBRACKET
                {
                    $$ = NULL;
                    yyerror("Rvariable error Array index");
                }
...
elsif_list      : elsif_list elsif
                {
                    ...
                    position_information *pos =
                        new position_information(@1.first_line,
                                                 @1.first_column);
                    $$ = new ast_elsif_list(pos, $2, $1); //(position_information*, ast_elsif*, ast_elsif_list*) add new elsif to list
                }
                | /* empty */
                {
                    /* Your code here */
