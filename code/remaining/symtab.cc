...
void symbol_table::open_scope()
{
    ...
    if (current_level == MAX_BLOCK)
        fatal("Cannot increase current level");
...
sym_index symbol_table::close_scope()
{
    /* Your code here */
    sym_index i;
    for(i=sym_pos; i>block_table[current_level];i--)
    {
        symbol *s = get_symbol(i);
        hash_index h = s->back_link;
        if(hash_table[h] == i)
        {
            hash_table[h] = s->hash_link;
...
sym_index symbol_table::lookup_symbol(const pool_index pool_p)
{
    ...
    sym_index i = hash_table[h];
    while (i!=NULL_SYM) {
        symbol *s = get_symbol(i);
        bool cmp = pool_compare(s->id, pool_p);
        if (cmp)
            return i;
        else
            i = s->hash_link;
                                       ...
                                       const sym_type tag)
{
    ...
    if(i!= NULL_SYM && sym_table[i]->level == current_level)
        return i;
    if(sym_pos >= MAX_SYM)
        fatal("Symbol table is filled");
    symbol *s = NULL;
    switch(tag)
    {
        case SYM_ARRAY: s = new array_symbol(pool_p);
                        break;
        case SYM_FUNC:  s = new function_symbol(pool_p);
                        break;
        case SYM_PROC:  s = new procedure_symbol(pool_p);
                        break;
        case SYM_VAR:   s = new variable_symbol(pool_p);
                        break;
        case SYM_PARAM: s = new parameter_symbol(pool_p);
                        break;
        case SYM_CONST: s = new constant_symbol(pool_p);
                        break;
        case SYM_NAMETYPE: s = new nametype_symbol(pool_p);
                           break;
        default : fatal("Unknown symbol type");
                  break;
    }
    sym_pos++;
    s->level = current_level;
    s->back_link = hash(pool_p);
    s->hash_link = hash_table[s->back_link];
    hash_table[s->back_link] = sym_pos;
    sym_table[sym_pos] = s;
