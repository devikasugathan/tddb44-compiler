                                ...
                                ast_expr_list *param_list)
{
    ...
    symbol *symb = sym_tab->get_symbol(call_id->sym_p);
   
    if(symb->tag == SYM_FUNC)
    {
        formals = symb->get_function_symbol()->last_parameter;
        chk_param(call_id,formals,param_list);

    }
    else if(symb->tag == SYM_PROC)
    {
        formals = symb->get_procedure_symbol()->last_parameter;
        chk_param(call_id,formals,param_list);

    }
    else {
        type_error(call_id->pos) << "Unable to call as it does not belong func/proc type. \n";
    }
...
{
    /* Your code here */
    //preceding->type_check(); last_expr->type_check();
    if (preceding != NULL) {
        preceding->type_check();
    }
    if (last_expr != NULL) {
...
{
    /* Your code here */
    array_symbol *temp = sym_tab->get_symbol(id->sym_p)->get_array_symbol();
    if(temp->tag != SYM_ARRAY)
    {
    ...
    if(index->type_check() != integer_type){
        ...
        type_error(pos) << "Index is not of integer type. \n";
...
sym_index semantic::check_binop1(ast_binaryoperation *node)
{
	...
	if (right_type == void_type || left_type == void_type){
		...
		type_error(node->pos) << "Void Error" << endl; 
        node->type = real_type;
		return real_type;
	}
...
sym_index ast_divide::type_check()
{
	...
	if (right_type == void_type || left_type == void_type){
		...
		type_error(pos) << "Void Error."<< endl;
...
sym_index semantic::check_binop2(ast_binaryoperation *node, string s)
{
    ...
    if(node->left->type_check() != integer_type){
        type_error(node->left->pos) << "Left value of  " << s << " should be an integer" << endl;
        return integer_type;
    }
    if(node->right->type_check() != integer_type){
        type_error(node->left->pos) << "Right value of " << s << " should be an integer" << endl;
        return integer_type;
    }
...
sym_index semantic::check_binrel(ast_binaryrelation *node)
{
    ...
    sym_index left_node = node->left->type_check();
    sym_index right_node = node->right->type_check();
    if(left_node == void_type || right_node == void_type){
        type_error(node->pos) << "Void type not alowed. \n";
        return void_type;
    }
    if(left_node != right_node){
        if(left_node == integer_type){
            node->left = new ast_cast(node->left->pos,node->left);
            node->left->type = real_type;
        }
        if(right_node == integer_type){
            node->right = new ast_cast(node->right->pos,node->right);
...
sym_index ast_not::type_check()
{
    ...
    if(expr->type_check()!= integer_type){
        type_error(expr->pos) << "Should be of type Integer. \n";
    }
    return integer_type;
}
...
sym_index ast_elsif::type_check()
{
    /* Your code here */
    if (condition->type_check() != integer_type) {
        type_error(condition->pos) << "Else if  must be of integer type.\n";
    }
